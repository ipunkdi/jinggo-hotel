<x-app-layout>
    ...
    <section class="bg-white dark:bg-gray-900">
        <div class="bg-white border border-gray-200 rounded-lg shadow py-8 px-4 mx-auto max-w-2xl lg:py-16">
            <h2 class="mb-4 text-xl font-bold text-gray-900 dark:text-white">Edit rate plan detail</h2>
            <form action="/sales/rate-plans/{{ $ratePlan->id }}" method="post">
                @method('put')
                @csrf
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <select id="type" name="unit_group_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500" disabled>
                            @foreach ($unitGroups as $unitGroup)
                                @if (old('unit_group_id', $ratePlan->unitGroup->id) == $unitGroup->id)
